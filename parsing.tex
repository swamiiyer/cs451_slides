...
\lstinline$#$ & \underline{\lstinline$id$}\lstinline$+id*id#$ &
\end{tabular}
\end{table}
...
\begin{tabular}{lll}
Stack & Input & Action \\ \hline \\
\lstinline$#$ & \underline{\lstinline$id$}\lstinline$+id*id#$ & shift \\
\lstinline$#id$ & \lstinline$+id*id#$ &
\end{tabular}
\end{table}
\end{frame}
...
\lstinline$#$ & \underline{\lstinline$id$}\lstinline$+id*id#$ & shift \\
\lstinline$#id$ & \underline{\lstinline$+$}\lstinline$id*id#$ & reduce (6) \\
\lstinline$#$\mm{F} & \underline{\lstinline$+$}\lstinline$id*id#$ & reduce (4) \\
\lstinline$#$\mm{T} & \underline{\lstinline$+$}\lstinline$id*id#$ & reduce (2) \\
...
The parser continues in this fashion, by a sequence of shifts and reductions, until we reach a configuration where \lstinline{#}$E$ is on the stack ($E$ on top) and the sole un-scanned symbol in the input is the terminator, \lstinline{#}
...
\lstinline$#$\mm{F} & \underline{\lstinline$+$}\lstinline$id*id#$ & reduce (4) \\
\lstinline$#$\mm{T} & \underline{\lstinline$+$}\lstinline$id*id#$ & reduce (2) \\
\lstinline$#$\mm{E} & \underline{\lstinline$+$}\lstinline$id*id#$ & shift \\
\lstinline$#$\mm{E}\lstinline$+$ & \underline{\lstinline$id$}\lstinline$*id#$ & shift \\
\lstinline$#$\mm{E}\lstinline$+id$ & \underline{\lstinline$*$}\lstinline$id#$ & reduce (6) \\
\lstinline$#$\mm{E}\lstinline$+$\mm{F} & \underline{\lstinline$*$}\lstinline$id#$ & reduce (4) \\
\lstinline$#$\mm{E}\lstinline$+$\mm{T} & \underline{\lstinline$*$}\lstinline$id#$ & shift \\
\lstinline$#$\mm{E}\lstinline$+$\mm{T}\lstinline$*$ & \underline{\lstinline$id$}\lstinline$#$ & shift \\
\lstinline$#$\mm{E}\lstinline$+$\mm{T}\lstinline$*id$ & \underline{\lstinline$#$} & reduce (6) \\
\lstinline$#$\mm{E}\lstinline$+$\mm{T}\lstinline$*$\mm{F} & \underline{\lstinline$#$} & reduce (3) \\
\lstinline$#$\mm{E}\lstinline$+$\mm{T} & \underline{\lstinline$#$} & reduce (1) \\
\lstinline$#$\mm{E} & \underline{\lstinline$#$} & accept
\end{tabular}
\end{table}
...
For example, in the derivation above, when the stack contains \lstinline{#}$E$\lstinline{+}$T$ and the next incoming token is a \lstinline{*}, how do we know that we are to shift (the \lstinline{*} onto the stack) rather than reduce either the $E$\lstinline{+}$T$ to an $E$ or the $T$ to an $E$?
\end{frame}
...
\begin{enumerate}
\item Ignoring the terminator \lstinline{#}, the stack configuration combined with the un-scanned input stream represents a sentential form in a right-most derivation of the input
...
The LR(1) parser algorithm is common to all LR(1) grammars and is driven by two tables (derived from a DFA that recognizes viable prefixes and handles): an Action table and a Goto table
