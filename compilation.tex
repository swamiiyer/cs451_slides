...
AST for the \lstinline{HelloWorld} program

\begin{center}
\visible<2->{\includegraphics[scale=0.3]{{figures/figure01.09}.jpg}}
\end{center}
\end{frame}
...
And to compile the program for the JVM machine, run the command
...
\pause\transdissolve
\item Implement type checking (aka semantic analysis) for the division operation

\pause\transdissolve
\item Implement code generation for the division operation
\end{enumerate}
...
Define a class \lstinline{JDivideOp} in \lstinline{JBinaryExpression.java} to represent an AST node for the division operator
...
\tcblower
\begin{minipage}[t][.25cm][t]{\textwidth}
analyze lhs and rhs operands of /
\end{minipage}
\end{tcolorbox}
...
\tcblower
\begin{minipage}[t][.25cm][t]{\textwidth}
generate code for lhs and rhs operands of /
\end{minipage}
\end{tcolorbox}
